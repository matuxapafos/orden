document.addEventListener("DOMContentLoaded", () => {
								...
								.map((request) => {
									const itemName =
										inventory.find((item) => item.id === request.itemId)
											?.name || "Неизвестно"
									const quantityOrDescription = request.quantity
										? request.quantity
										: request.description
												...
												<td>
													<select class="request-status-select" data-request-id="${request.id}">
															<option value="ожидает" ${
																request.status === "ожидает" ? "selected" : ""
															}>ожидает</option>
												 ...
												 <td>
													<button class="delete-request-button" data-request-id="${
														request.id
													}">Удалить</button>
											 </td>
										</tr>
											 ...
											 <td>${item.quantity}</td>
												<td>
														 <select class="status-select" data-item-id="${item.id}">
															 <option value="новый" ${
																	item.status === "новый" ? "selected" : ""
																}>новый</option>
															 <option value="используемый" ${
																	item.status === "используемый"
																		? "selected"
																		: ""
																}>используемый</option>
																 <option value="сломанный" ${
		...
		return `
					...
					<tbody>
							${purchasePlans
								.map((plan) => {
									const itemName =
										inventory.find((item) => item.id === plan.itemId)?.name ||
										"Неизвестно"
									return `
										 <tr>
											<td>${plan.id}</td>
	...
	function renderInventoryReport() {
		...
		const usedItems = inventory.filter(
			(item) => item.status === "используемый"
		).length
		const brokenItems = inventory.filter(
			(item) => item.status === "сломанный"
		).length
